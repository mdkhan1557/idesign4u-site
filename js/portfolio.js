...
/* ────────────────────────────────────────────────────────────
   ...
   CHANGED: opacity range raised (0.18–0.38), size range
   increased (0.9–1.7rem), duration slowed (9–18s),
   count raised on desktop (38 icons).
   ──────────────────────────────────────────────────────────── */
(function spawnIcons() {
  var labels = [
    '{ }', '</>', '<html>', '<div>', '#id', '.class',
  ...
  // CHANGED: more icons, especially on desktop
  var count = window.innerWidth < 600 ? 20 : 38;
  ...
  for (var i = 0; i < count; i++) {
    ...
    // CHANGED: opacity floor raised from .06 to .18, ceiling from .18 to .38
    var opacity = (0.18 + Math.random() * 0.20).toFixed(3);

    // CHANGED: font-size range raised from 0.65–1.5rem to 0.9–1.7rem
    var size = (0.9 + Math.random() * 0.8).toFixed(2);

    // CHANGED: slower animation duration (was 7–18s, now 9–20s)
    var dur = (9 + Math.random() * 11).toFixed(1);
...
/* ────────────────────────────────────────────────────────────
   2. RISING PARTICLES
   CHANGED: size range raised (3–9px was 2–7px), opacity
   colours raised, count increased to 28 desktop / 12 mobile.
   ──────────────────────────────────────────────────────────── */
(function spawnParticles() {
  ...
  // CHANGED: more particles
  var count = window.innerWidth < 600 ? 12 : 28;

  for (var i = 0; i < count; i++) {
    var el = document.createElement('div');
    el.className = 'pf-particle';

    // CHANGED: bigger dots (3–9px, was 2–7px)
    ...
    el.style.cssText = [
      ...
      'left:' + left + '%;',
      'color:' + col + ';',   // used by box-shadow: currentColor in CSS
      'background:' + col + ';',
      'animation-duration:' + dur + 's;',
      'animation-delay:' + delay + 's;',
      '--drift:' + drift + 'px;'
